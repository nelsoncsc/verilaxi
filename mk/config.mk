...
ifeq ($(TESTNAME),axis_register)
  ...
  RUN_TAG := $(TESTNAME)$(VIDEO_VALIDATE_TAG)
else ifeq ($(TESTNAME),video_rgb32)
  RUN_TAG := $(TESTNAME)
else ifeq ($(TESTNAME),video_csc_rgb_ycbcr)
  RUN_TAG := $(TESTNAME)
else ifeq ($(TESTNAME),video_csc_422)
  RUN_TAG := $(TESTNAME)
...
ifeq ($(TESTNAME),axis_register)
    ...
    ENV_FILE := $(TB_DIR)/tests/video/test_video_rgb32.sv
else ifeq ($(TESTNAME),video_csc_rgb_ycbcr)
    ENV_FILE := $(TB_DIR)/tests/video/test_video_csc_rgb_ycbcr.sv
else ifeq ($(TESTNAME),video_csc_422)
    ENV_FILE := $(TB_DIR)/tests/video/test_video_csc_422.sv
else ifeq ($(TESTNAME),dma)
...
SIM_ARGS += $(if $(VDMA_STRESS),+VDMA_STRESS=$(VDMA_STRESS))
...
ifeq ($(TESTNAME),axis_register)
  ...
  VERILATOR_DEFS := +define+USE_VIDEO_RGB32
else ifeq ($(TESTNAME),video_csc_rgb_ycbcr)
  VERILATOR_DEFS := +define+USE_VIDEO_CSC_RGB_YCBCR
else ifeq ($(TESTNAME),video_csc_422)
  VERILATOR_DEFS := +define+USE_VIDEO_CSC_422
else ifeq ($(TESTNAME),dma)
