...
// FRAME_FIFO is a compile-time define (+define+FRAME_FIFO).
// When set, the FIFO operates in store-and-forward mode: m_axis_tvalid is
// suppressed until the full packet (up to tlast) has been written in.
`ifdef FRAME_FIFO
  `define AXIS_FIFO_FRAME_MODE 1'b1
`else
  `define AXIS_FIFO_FRAME_MODE 1'b0
`endif

module test_axis_fifo #(parameter int DATA_WIDTH = 8,
                        parameter int USER_WIDTH = 1)
                       (input logic clk,
                        input logic rst_n);
                        import axi_pkg::*;

    localparam bit FRAME_MODE = `AXIS_FIFO_FRAME_MODE;
    ...
    initial begin
        ...
        $display("AXIS FIFO: SRC_BP=%0d SINK_BP=%0d FRAME_FIFO=%0d",
    ...
    snix_axis_fifo #(.DATA_WIDTH  (DATA_WIDTH),
                     .USER_WIDTH  (USER_WIDTH),
                     .FIFO_DEPTH  (FIFO_DEPTH),
                     .FRAME_FIFO  (FRAME_MODE))
                   axis_fifo_u0 (.clk          (clk),
                                 .rst_n        (rst_n),
    ...
    initial begin
        #1_000_000;
        $fatal("test_axis_fifo: simulation timeout");
    end

endmodule: test_axis_fifo
