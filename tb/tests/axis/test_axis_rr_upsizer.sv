...
// -------------------------------------------------
// test_axis_rr_upsizer
//
// Exercises snix_axis_rr_upsizer with IN=16, OUT=24  (3:2 ratio).
...
//
//   3 in  → 2 out   full group          tkeep = 3'b111
//   1 in  → 1 out   TLAST at beat 0     tkeep = 3'b011  (2 valid bytes)
//   2 in  → 2 out   TLAST at beat 1     tkeep = 3'b001  (1 valid byte)
//   6 in  → 4 out   two full groups     tkeep = 3'b111
//
...
module test_axis_rr_upsizer #(
    ...
    initial begin
        ...
        $display("[RRU] SRC_BP=%0d SINK_BP=%0d IN=%0d OUT=%0d",
    ...
    // Protocol checkers
    // -------------------------------------------------------
    axis_checker #(.DATA_WIDTH(IN_DATA_WIDTH),  .USER_WIDTH(1), .LABEL("RRU_SLV"))
        u_chk_s (.clk(clk), .rst_n(rst_n),
                 .tdata(s_tdata_w), .tuser(1'b0),
                 .tvalid(s_tvalid_w), .tready(s_tready_w), .tlast(s_tlast_w));

    axis_checker #(.DATA_WIDTH(OUT_DATA_WIDTH), .USER_WIDTH(1), .LABEL("RRU_MST"))
    ...
    initial begin
        ...
        forever begin
            ...
            if (m_tvalid_w && m_tready_w) begin
                ...
                logic exp_last;
                $display("[RRU] beat %0d tdata=0x%h tkeep=%03b tlast=%0b",
                ...
                if (m_tkeep_w !== exp_keep)
                    $error("[RRU] FAIL: beat %0d expected tkeep=%03b, got %03b",
                           beats_recv, exp_keep, m_tkeep_w);

                if (m_tlast_w !== exp_last)
                    $error("[RRU] FAIL: beat %0d expected tlast=%0b, got %0b",
                ...
                for (int lane = 0; lane < valid_bytes; lane++) begin
                    if (m_tdata_w[lane*8 +: 8] !== exp_bytes[exp_byte_idx + lane]) begin
                        $error("[RRU] FAIL: beat %0d lane %0d expected byte 0x%02h, got 0x%02h",
    ...
    initial begin
        ...
        $display("[RRU] ---- Phase 1: Sequential ----");
        send_pkt(3, 1);   // full group        → 2 out-beats, tkeep_last=111
        send_pkt(1, 1);   // TLAST at beat 0   → 1 out-beat,  tkeep_last=011
        send_pkt(2, 1);   // TLAST at beat 1   → 2 out-beats, tkeep_last=001
        send_pkt(6, 1);   // two full groups   → 4 out-beats, tkeep_last=111
        wait (pkts_recv == 4);
        $display("[RRU] Phase 1 done: %0d beats / %0d pkts", beats_recv, pkts_recv);

        $display("[RRU] ---- Phase 2: Backpressure ----");
        send_pkt(3, 1);
        send_pkt(1, 1);
        send_pkt(2, 1);
        send_pkt(6, 1);
        ...
        $display("[RRU] Phase 2 done: %0d beats / %0d pkts", beats_recv, pkts_recv);

        if (beats_recv !== exp_beats)
            $error("[RRU] FAIL: expected %0d beats, got %0d", exp_beats, beats_recv);
        else if (pkts_recv !== exp_pkts)
            $error("[RRU] FAIL: expected %0d packets, got %0d", exp_pkts, pkts_recv);
        else if (exp_byte_idx !== exp_bytes.size())
            $error("[RRU] FAIL: consumed %0d expected bytes, expected %0d",
                   exp_byte_idx, exp_bytes.size());
        else
            $display("[RRU] PASS: %0d beats / %0d pkts, byte-accurate scoreboard matched",
    ...
    initial begin
        #2_000_000;
        $fatal("test_axis_rr_upsizer: simulation timeout");
    end

endmodule: test_axis_rr_upsizer
