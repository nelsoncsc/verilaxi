...
3. [Module Reference](#3-module-reference)
   ...
   - 3.4–3.5 Video Infrastructure & VDMA → **[VIDEO.md](VIDEO.md)**
4. [DMA, CDMA, and VDMA CSR Register Maps](#4-dma-cdma-and-vdma-csr-register-maps)
   ...
   - 4.3 VDMA Register Map → **[VIDEO.md §5–6](VIDEO.md#5-vdma-register-map)**
...
### 3.4 Video Infrastructure & 3.5 AXI Video DMA

All video material — the pixel-clock infrastructure modules, single-stream VDMA,
the multi-tap temporal VDMA, and the PNG/`stb_image` image harness — lives in a
dedicated reference: **[VIDEO.md](VIDEO.md)**.
...
### 4.3 VDMA Register Map

The full `snix_axi_vdma` register map (WR/RD control, STATUS, geometry,
FRAME_ADDR, FRAME_CTRL, IRQ_ACK) and the multi-tap `snix_axi_multi_vdma` CSR map
are documented in **[VIDEO.md §5–6](VIDEO.md#5-vdma-register-map)**.
...
### 6.8 PNG Frame Harness

The DPI-C / `stb_image` real-image harness (`video_frame_source` /
`video_frame_sink`, `tb_cpp/video_frame_dpi.cpp`, and the `vf_*` entry points) is
documented in **[VIDEO.md §7](VIDEO.md#7-png-frame-harness-dpi-c--stb_image)**.
...
| `video_rgb32`           | —    | `tb/tests/video/test_video_rgb32.sv`             | RGB32 (XRGB8888) pack/unpack round-trip |
| `video_csc_rgb_ycbcr`   | —    | `tb/tests/video/test_video_csc_rgb_ycbcr.sv`     | RGB↔YCbCr BT.601 round-trip (bounded error) |
| `video_csc_422`         | —    | `tb/tests/video/test_video_csc_422.sv`           | 4:4:4↔4:2:2 chroma subsample (YUYV/UYVY) |
| `vdma_timing`           | —    | `tb/tests/axi/test_vdma_timing.sv`               | VDMA with `pixel_clk ≠ axi_clk`; per-frame PNG verify |
...
make run TESTNAME=video_rgb32
make run TESTNAME=video_csc_rgb_ycbcr
make run TESTNAME=video_csc_422

# VDMA with an independent pixel clock (74.25 MHz pixel vs 200 MHz AXI)
make run TESTNAME=vdma_timing
make run TESTNAME=vdma_timing VIDEO_VALIDATE=1      # 32x16 colour-bar profile

# Drive real PNG frames through the pipeline (squirrel round-trip) — see VIDEO.md §8
...
**VDMA and video infrastructure tests** — see **[VIDEO.md](VIDEO.md)** for the full pipeline, both VDMA engines, and the image harness.
